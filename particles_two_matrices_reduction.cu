...
__global__ void ForceCompt_par(double *f, double *x, double *y,
    ...
    if (row < np && col < np/2) {
            ...
            if (col != row) {
                dx = x[row] - x[col];
                dy = y[row] - y[col];
                // to avoid splitting control flow with if/else
                //d2 = int(d2 >= tiny) * (dx * dx + dy * dy) + int(d2 < tiny) * (tiny);
...
int main(int argc, char *argv[]) {
    ...
    HANDLE_ERROR(
            cudaMalloc((void **)&forceCompt_dev, sizeof(double) * Particles.np * Particles.np / 2));
    //HANDLE_ERROR(
    ...
    int N = Particles.np/2;
    dim3 dimBlock(TILE_WIDTH, TILE_WIDTH);
    dim3 dimGrid(2*N / TILE_WIDTH + 1, N / TILE_WIDTH + 1);
    int dimBlock1d = TILE_WIDTH;
    int dimGrid1d = 2*N / TILE_WIDTH + 1;
    for (int k = 0; k < MaxSteps; k++) {
        HANDLE_ERROR(cudaMemset(forces_dev, 0, 2 * sizeof(double) * Particles.np));
        // compute only half of the matrix (is inside the kernel)
        ForceCompt_par<<<dimGrid, dimBlock>>>(forceCompt_dev, posX_dev, posY_dev,
                weight_dev, Particles.np, 1);
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ReduceForce<<<dimGrid1d, dimBlock1d>>>(forces_dev, forceCompt_dev, Particles.np/2, 1);
        // compute the other half of the matrix, passing a pointer to the middle of the array
        
        ForceCompt_par<<<dimGrid, dimBlock>>>(forceCompt_dev, &posX_dev[Particles.np/2], &posY_dev[Particles.np/2],
                &weight_dev[Particles.np/2], Particles.np, 1);
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ReduceForce<<<dimGrid1d, dimBlock1d>>>(forces_dev, forceCompt_dev, Particles.np/2, 1);
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ...
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ReduceForce<<<dimGrid1d, dimBlock1d>>>(forces_dev, forceCompt_dev, Particles.np/2, 0);
        // compute other half
        ForceCompt_par<<<dimGrid, dimBlock>>>(forceCompt_dev, &posX_dev[Particles.np/2], &posY_dev[Particles.np/2],
                &weight_dev[Particles.np/2], Particles.np, 0);
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ReduceForce<<<dimGrid1d, dimBlock1d>>>(forces_dev, forceCompt_dev, Particles.np/2, 0);
