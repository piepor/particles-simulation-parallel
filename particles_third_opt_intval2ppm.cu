...
// Parameters
int** colorMap; // 3x256 matrix of RGB colors
...
__device__ double MaxIntVal_dev(int s, int *a);
__device__ double MinIntVal_dev(int s, int *a);
double MaxDoubleVal(int s, double *a);
double MinDoubleVal(int s, double *a);
// display functions
void read_color_map(int **cm);
__device__ void IntVal2ppm_dev(int *idata_dev, int np, int rmin, int rmax);
void scrivi_vp(int* vp_matrix, char *name);
...
void ParticleScreen(struct i2dGrid *pgrid, struct Population pp, int step) {
    ...
    if (step <= 0) {
        read_color_map(colorMap);
        vmin = vmax = 0;
    }

    // CALCOLO DI RMIN/RMAX e (VMIN/VMAX per lo step successivo)
    /*  Values from 0 to 255 */
    if ((vmin == vmax) && (vmin == 0)) {
        // alla prima invocazione di IntVal2ppm (step 0), vim=vmax=0
        // setta vmax/vmin (puntatori) a rmax/rmin
        rmin = MinIntVal(s1 * s2, pgrid->Values);   // min value of matrix (o MinIntVal_dev)
        rmax = MaxIntVal(s1 * s2, pgrid->Values);   // max value of matrix (o MaxIntVal_dev)
        vmin = rmin;
        vmax = rmax;
    } else {
        // vmin/vmax sono int static nel chiamante, perciò non vengono mai azzerate
        rmin = vmin;
        rmax = vmax;
    }

    //    int* value_dev;
    //    cudaMalloc value_dev
    //    cudaMemcpy HOST-> DEVICE di pgrid->Values
    
    // idata_dev è 1200 * 1000 = 1,2 Mcelle
    // posso fare:
    // 9375 blocchi da 64 threads ciascuno. Permetterebbe di caricare 32 blocchi alla volta in ogni SM,
    // occupando 2048 threads alla volta == il 100% dei threads disponibili per SM.
    IntVal2ppm_dev<<<dimGrid(9375),dimBlock(64)>>>(value_dev, pp.np, rmin, rmax);

    int* temp_values = (int*) malloc(sizeof(int) * pgrid->EX * pgrid->EY);
    //    cudaMemcpy DEVICE -> HOST di value_dev -> temp_values
    //    cudaFree value_dev
    scrivi_vp(temp_values, name);
    free(temp_values);
}

void read_color_map(int **cm){      // cm is a 3x256 matrix
    /* Call this to inizialize cm. You need to do it just once. */
    FILE *ColMap;

    // LETTURA DEL FILE COLORMAP IN MATRICE cm
    /*
...
__device__ void IntVal2ppm_dev(int *idata_dev, int np, int rmin, int rmax) {
   ...
   */
    ...
    if (tid < np) {
        int value = idata_dev[tid]  // leggi dati
        if ( value < rmin ) 
            value = rmin;
        if ( value > rmax ) 
            value = rmax;
        // sovrascriviamo idata_dev con il valore calcolato (vp)
        idata_dev[tid] = (int)((double)(value - rmin) * (double) 255.0 / (double) (rmax - rmin));
    }
}
...
void scrivi_vp(int* vp_matrix, char *name) {
   ...
   */
    ...
    vs = 0;
    for (int i = 0; i < ParticleGrid.EX; i++) {
        for (int j = 0; j < ParticleGrid.EY; j++) {
            vp = vp_matrix[i * ParticleGrid.EY + j];
            vs++;
            fprintf(ouni, " %3.3d %3.3d %3.3d", colorMap[0][vp], colorMap[1][vp], colorMap[2][vp]);
            if (vs >= 10) {
                fprintf(ouni, " \n");
                vs = 0;
            }
