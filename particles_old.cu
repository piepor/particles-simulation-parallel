...
int main(int argc, char *argv[]) /* FinalApplication */ {
  ...
  int *deviceValues;
  int *valuesArr;
  valuesArr = (int*) malloc(sizeof(int)*GenFieldGrid.EX * GenFieldGrid.EY);
  ...
  printf("EX %d\n", GenFieldGrid.EX);
  printf("EY %d\n", GenFieldGrid.EY);
  HANDLE_ERROR(cudaMemcpy(deviceValues, valuesArr, sizeof(int) * 2400 * 2000, cudaMemcpyHostToDevice));
  HANDLE_ERROR(cudaMemcpy(GenFieldGrid_dev, &GenFieldGrid, sizeof(struct i2dGrid), cudaMemcpyHostToDevice));
  GeneratingField<<<dimGrid, dimBlock>>>(GenFieldGrid_dev, deviceValues, MaxIters);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
    printf("CUDA error: %s\n", cudaGetErrorString(err));
  HANDLE_ERROR(cudaMemcpy(valuesArr, deviceValues, sizeof(int) * 2400 * 2000, cudaMemcpyDeviceToHost));
