...
__global__ void ForceCompt_par_x(double *f, double *x, double *y, double *weight,
                               int np) {
    ...
    if (tid < np) {
        for (int i = 0; i < np; i++) {
            if (i != tid) {
                ...
                //if (d2 < tiny)
                    //d2 = tiny;
                force = int(d2 > tiny) * (k * weight[tid] * weight[i]) / d2;
                fx = force * dx / sqrt(d2);
...
__global__ void ForceCompt_par_y(double *f, double *x, double *y, double *weight,
                               int np) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    double force, d, d2, dx, dy, fx, fy;
    static double k = 0.001, tiny = (double)1.0 / (double)1000000.0;
...
int main(int argc, char *argv[]) {
    ...
    for (int k = 0; k < MaxSteps; k++) {
        ...
        ForceCompt_par_x<<<dimGrid, dimBlock, 1, stream1>>>(forces_dev, posX_dev, posY_dev,
            weight_dev, Particles.np);
        ForceCompt_par_y<<<dimGrid, dimBlock, 1, stream2>>>(forces_dev, posX_dev, posY_dev,
            weight_dev, Particles.np);
