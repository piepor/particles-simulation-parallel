...
__global__ void ForceCompt_par_x(double *f, double *x, double *y,
        double *weight, int np) {
    ...
    //while (row < np && col < np) {
            if (col != row) {
                ...
                d2 = dx * dx +
                     dy * dy; // what if particles get in touch? Simply avoid the case
                force = int(d2 > tiny) * (k * weight[col] * weight[row]) / d2;
...
__global__ void ForceCompt_par_y(double *f, double *x, double *y,
        double *weight, int np) {
    int col = blockIdx.x * blockDim.x + threadIdx.x;
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    double force, d, d2, dx, dy, fcompt;
...
__global__ void ReduceForce_x(double *f, double *fcompt, int np){
...
__global__ void ReduceForce_y(double *f, double *fcompt, int np){
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    double totForce = 0;
...
int main(int argc, char *argv[]) {
    ...
    double *weight_dev_y;
    double *forces_dev;
    double *forceComptX_dev;
    double *forceComptY_dev;
    //double *forceY_dev;
    HANDLE_ERROR(cudaMalloc((void **)&posX_dev_x, sizeof(double) * Particles.np));
    HANDLE_ERROR(cudaMalloc((void **)&posY_dev_x, sizeof(double) * Particles.np));
    HANDLE_ERROR(cudaMalloc((void **)&posX_dev_y, sizeof(double) * Particles.np));
    HANDLE_ERROR(cudaMalloc((void **)&posY_dev_y, sizeof(double) * Particles.np));
    ...
    HANDLE_ERROR(cudaMalloc((void **)&weight_dev_x, sizeof(double) * Particles.np));
    HANDLE_ERROR(cudaMalloc((void **)&weight_dev_y, sizeof(double) * Particles.np));
    // allocate space for the force array
    // NEW CODE WITH REDUCTION AND FORCES MATRICES
    HANDLE_ERROR(
            cudaMalloc((void **)&forceComptX_dev, sizeof(double) * Particles.np * Particles.np));
    HANDLE_ERROR(
            cudaMalloc((void **)&forceComptY_dev, sizeof(double) * Particles.np * Particles.np));
    //HANDLE_ERROR(
    ...
    HANDLE_ERROR(cudaMemcpy(posX_dev_x, Particles.x, sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(posY_dev_x, Particles.y, sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(posX_dev_y, Particles.x, sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(posY_dev_y, Particles.y, sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    ...
    HANDLE_ERROR(cudaMemcpy(velY_dev, Particles.vy, sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(weight_dev_x, Particles.weight,
                            sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    HANDLE_ERROR(cudaMemcpy(weight_dev_y, Particles.weight,
                            sizeof(double) * Particles.np,
                            cudaMemcpyHostToDevice));
    ...
    for (int k = 0; k < MaxSteps; k++) {
        HANDLE_ERROR(cudaMemset(forceComptX_dev, 0, sizeof(double) * Particles.np *  Particles.np));
        HANDLE_ERROR(cudaMemset(forceComptY_dev, 0, sizeof(double) * Particles.np *  Particles.np));
        HANDLE_ERROR(cudaDeviceSynchronize());
        //HANDLE_ERROR(cudaMemset(forces_dev, 0, 2 * sizeof(double) * Particles.np));
        ForceCompt_par_x<<<dimGrid, dimBlock, 1, stream1>>>(forceComptX_dev, posX_dev_x, posY_dev_x,
                weight_dev_x, Particles.np);
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ForceCompt_par_y<<<dimGrid, dimBlock, 1, stream2>>>(forceComptY_dev, posX_dev_y, posY_dev_y,
                weight_dev_y, Particles.np);
        ReduceForce_x<<<dimGrid1d, dimBlock1d, 1, stream1>>>(forces_dev, forceComptX_dev, Particles.np);
        //HANDLE_ERROR(cudaDeviceSynchronize());
        //HANDLE_ERROR(cudaDeviceSynchronize());
        ReduceForce_y<<<dimGrid1d, dimBlock1d, 1, stream2>>>(forces_dev, forceComptY_dev, Particles.np);
//        HANDLE_ERROR(cudaMemcpy(forces, forces_dev,
//                                sizeof(double) * 2 * Particles.np,
//                                cudaMemcpyDeviceToHost));
        HANDLE_ERROR(cudaDeviceSynchronize());
        ComptPopulation_par<<<dimGrid1d, dimBlock1d>>>(posX_dev_x, posY_dev_x, velX_dev,
                                                   velY_dev, forces_dev, weight_dev_x,
...
//        DumpForces(forces, k, Particles.np, "par_forces\0");
        ...
        HANDLE_ERROR(cudaDeviceSynchronize());
        HANDLE_ERROR(cudaMemcpy(posX_dev_y, posX_dev_x,
                                sizeof(double) * Particles.np,
                                cudaMemcpyDeviceToDevice));
        HANDLE_ERROR(cudaMemcpy(posY_dev_y, posY_dev_x,
                                sizeof(double) * Particles.np,
                                cudaMemcpyDeviceToDevice));
        HANDLE_ERROR(cudaMemcpy(Particles.x, posX_dev_x,
                                sizeof(double) * Particles.np,
                                cudaMemcpyDeviceToHost));
        HANDLE_ERROR(cudaMemcpy(Particles.y, posY_dev_x,
                                sizeof(double) * Particles.np,
                                cudaMemcpyDeviceToHost));
        ...
        HANDLE_ERROR(cudaMemcpy(Particles.vy, velY_dev,
                                sizeof(double) * Particles.np,
                                cudaMemcpyDeviceToHost));
        HANDLE_ERROR(cudaMemcpy(Particles.weight, weight_dev_x,
                                sizeof(double) * Particles.np,
                                cudaMemcpyDeviceToHost));
    ...
    HANDLE_ERROR(cudaFree(posX_dev_x));
    HANDLE_ERROR(cudaFree(posY_dev_x));
    HANDLE_ERROR(cudaFree(posX_dev_y));
    HANDLE_ERROR(cudaFree(posY_dev_y));
    HANDLE_ERROR(cudaFree(velX_dev));
    HANDLE_ERROR(cudaFree(velY_dev));
    HANDLE_ERROR(cudaFree(weight_dev_x));
    HANDLE_ERROR(cudaFree(weight_dev_y));
    HANDLE_ERROR(cudaFree(forces_dev));
    HANDLE_ERROR(cudaFree(forceComptX_dev));
    HANDLE_ERROR(cudaFree(forceComptY_dev));
